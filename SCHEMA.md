...
| **Avg days to first response** | Mean, over applications with ≥ 2 stages, of `firstResponseAt − appliedDate` in days — where `firstResponseAt` is the earliest non-null of `scheduledAt` / `completedAt` on the stage with the **lowest `sequence` greater than 1**. (`sequence` is 1-based; don't read this as a 0-indexed array offset.) **This can go negative, and did.** Nothing constrains a stage to postdate `appliedDate`, so an application whose applied date is later than a round it already has drags the mean below zero — real data produced −11.3 days for one application and an MCP tool reported "Average time to first response: -11.1 days" (2026-09-02). Fixed by correcting the data, **not** by clamping: a negative here is a true statement about contradictory input, and flooring it at zero would hide a data-entry error behind a plausible number. If it recurs, check `appliedDate` against `stages[]` before touching this definition. |
